...
class GeneExpressionQueryManager:
    ...
    def setup_normalization_data_method_2(self, gene_id_norm, df_snaptron_results_norm, meta_data_df):
            ...
            for each_sample in sample.split(','):
                if each_sample:
                    ...
                    try:  # you need the try because railID may not be in the metadata
                        ...
                        study = (meta_data_df.loc[rail_id])['study']
                        t = (rail_id, count)
    ...
    def setup_normalization_data_method_2_opt(self, gene_id_norm, df_snaptron_results_norm, meta_data_dict):
    ...
    def _gather_rail_id_meta_data(self, rail_id, meta_data_df):
        ...
        try:
            ...
            meta_data = (meta_data_df.loc[rail_id]).to_dict()
    ...
    def run_gene_expression_query(self, gene_id_query, df_snaptron_results_query, meta_data_df):
        ...
        for rail_id in self.rail_id_dictionary:
            self._gather_rail_id_meta_data(rail_id, meta_data_df)

        return self.gathered_rail_id_meta_data_and_counts

    def run_gene_expression_query_opt(self, gene_id_query, df_snaptron_results_query, meta_data_dict):
        # extract the row in the results that matches the query gene ID
        row_df = df_snaptron_results_query.loc[
            df_snaptron_results_query[gs.snpt_col_gene_id].str.contains(gene_id_query)]
