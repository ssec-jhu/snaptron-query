...
# 1 ND 3 SEEM SIMILAR? THE DTAFRAME CREATION STEP IS INSIGNIFICANT
workingList = 1
...
class JunctionInclusionQueryManager:
    ...
    @staticmethod
    def gather_rail_id_and_counts(samples, mark_in_or_ex, rail_id_dictionary):
        ...
        for junction_samples in samples:
            ...
            for eachSample in js:
                if eachSample:
                    ...
                    if rail_id in rail_id_dictionary:
                        rail_id_dictionary[rail_id].append(dict_value)
                    else:
                        rail_id_dictionary[rail_id] = [dict_value]

        return rail_id_dictionary

    def __gather_rail_id_meta_data(self, df_meta_data, rail_id_dictionary, rail_id_meta_data_list, railID):
        ...
        try:
            ...
            if (indexing):
                ...
                meta_data_series = df_meta_data.loc[int(railID)]  # NEED to cast to int or it won't work!
            else:
                # without indexing this returns a data FRAME
                df_data = df_meta_data.loc[df_meta_data['rail_id'] == int(railID)]
            ...
            counts = rail_id_dictionary[railID]
            inclusion_count = 0
            exclusion_count = 0
            total_count = 0
            psi = 0.0
            ...
            # TODO: PSI calculation tolerance of 15?
            if (total_count >= 0):
                ...
                if (workingList == 1):
                    data = f'{railID},{external_id},{study},{inclusion_count},{exclusion_count},{total_count},{psi}'
                    rail_id_meta_data_list.append(data)
                elif (workingList == 2):
                    data = [f'{railID},{study},{inclusion_count}']
                    rail_id_meta_data_list.append(data)
                elif (workingList == 3):
                    r = f'{railID}'
                    s = f'{study}'
            ...
            print(f"{railID} not found in samples. Moving on to the next iteration.")


        return rail_id_meta_data_list

    def run_junction_inclusion_query(self, df):
        # TODO: move al strings into a strings file
        ...
        # TODO: check for df size?

        # extract the 'sample' column form the row
        # this is where all the samples nd their count is
        exclusion_junction_samples = (exc_junctions_df['samples']).tolist()
        inclusion_junction_samples = (inc_junctions_df['samples']).tolist()

        # gather results in a dictionary. samples that were extracted as part of an inclusion junction are marked
        # as 'true'
        rail_id_dictionary = {}
        ...
        # TODO: this function should be called in global space and its data frame passed in based on compilation
        ...
        if (lookupInDictionary == 1):
            for railID in rail_id_dictionary:
                rail_id_data_list = self.__gather_rail_id_meta_data(df_meta_data, rail_id_dictionary, rail_id_data_list,
                                                                    railID)
        elif (lookupInDictionary == 2):
            for railID in rail_id_dictionary.keys():
                rail_id_data_list = self.__gather_rail_id_meta_data(df_meta_data, rail_id_dictionary, rail_id_data_list,
                                                                    railID)
        elif (lookupInDictionary == 3):
            keyList = list(rail_id_dictionary.keys())
            for railID in keyList:
                rail_id_data_list = self.__gather_rail_id_meta_data(df_meta_data, rail_id_dictionary, rail_id_data_list,
                                                                    railID)
